/**
 * Bolt Fleet Integration API — proxy seguro para Cloudflare Workers.
 *
 ...
 *   ALLOWED_ORIGINS=https://teu-dominio.pt,https://www.teu-dominio.pt
 *   REQUIRE_CF_ACCESS=true
 *   ALLOW_NO_ORIGIN=false
 *
 * IMPORTANTE:
 * - Protege a rota do Worker com Cloudflare Access. CORS não é autenticação.
 * - Nunca coloques BOLT_CLIENT_SECRET no HTML, JavaScript do browser ou GitHub.
 */
...
function validateEnvironment(env) {
  ...
  if (!allowedOrigins(env).length && !parseBoolean(env.ALLOW_NO_ORIGIN, false)) {
    missing.push('ALLOWED_ORIGINS');
  }
  if (missing.length) {
    throw new HttpError(500, 'CONFIGURATION_ERROR', `Configuração em falta: ${missing.join(', ')}.`);
  }
}

function enforceCloudflareAccess(request, env) {
  if (!parseBoolean(env.REQUIRE_CF_ACCESS, false)) return;

  // Esta verificação é apenas defesa adicional. A aplicação Cloudflare Access
  // tem de estar efetivamente configurada para proteger o domínio/rota do Worker.
  const jwt = request.headers.get('Cf-Access-Jwt-Assertion');
  if (!jwt) {
    throw new HttpError(401, 'ACCESS_REQUIRED', 'É necessário autenticar através do Cloudflare Access.');
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (!cors.ok) {
      return jsonResponse({ ok: false, error: { code: 'ORIGIN_NOT_ALLOWED', message: cors.reason }, request_id: requestId }, 403, {});
    }
    ...
    try {
      ...
      enforceCloudflareAccess(request, env);
      ...
      return jsonResponse(data, 200, cors.headers, { 'X-Request-Id': requestId });
    } catch (error) {
      ...
      const payload = {
        ...
        error: {
          ...
          message: isHttpError ? error.message : 'Erro interno no proxy Bolt.',
      ...
      return jsonResponse(payload, status, cors.headers, { 'X-Request-Id': requestId });
    }
  },
};
